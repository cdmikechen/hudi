...
import static org.apache.hudi.hadoop.utils.HoodieHiveUtils.HIVE_DEFAULT_TIMESTAMP_LOGICAL_TYPE_VALUE;
import static org.apache.hudi.hadoop.utils.HoodieHiveUtils.HUDI_TIMESTAMP_LOGICAL_TYPE_KEY;

@SerDeSpec(schemaProps = {serdeConstants.LIST_COLUMNS, serdeConstants.LIST_COLUMN_TYPES,
        ParquetOutputFormat.COMPRESSION})
public class HudiParquetAvroSerDe extends AbstractSerDe {
  ...
  public final void initialize(final Configuration conf, final Properties tbl) throws SerDeException {
    ...
    this.objInspector = new HudiArrayWritableObjectInspector(completeTypeInfo, prunedTypeInfo,
            tbl.getProperty(HUDI_TIMESTAMP_LOGICAL_TYPE_KEY, HIVE_DEFAULT_TIMESTAMP_LOGICAL_TYPE_VALUE));

    // Stats part
    serializedSize = 0;
    deserializedSize = 0;
    status = LASTOPERATION.UNKNOWN;
  }

  @Override
  public Object deserialize(final Writable blob) throws SerDeException {
    status = LASTOPERATION.DESERIALIZE;
    deserializedSize = 0;
  ...
  public Writable serialize(final Object obj, final ObjectInspector objInspector)
    ...
    status = LASTOPERATION.SERIALIZE;
  ...
  public SerDeStats getSerDeStats() {
    // must be different
    assert (status != LASTOPERATION.UNKNOWN);
    if (status == LASTOPERATION.SERIALIZE) {
  ...
  private static void pruneFromSinglePath(PrunedStructTypeInfo prunedInfo, String path) {
    if (prunedInfo == null) {
      throw new IllegalArgumentException("PrunedStructTypeInfo for path '" + path + "' should not be null");
    }
